...
class MetaData:
    ...
    @staticmethod
    def get_extraction_info(wait, driver, time_start, time_stop, input_last_page, current_last_page):
        ...
        extracted_ads = f"{current_last_page} Anúncios extraídos de {ads_quantity}"
        extraction_name_data = f"{datetime.date.today()}"
        extraction_name_hour = f"{datetime.datetime.now().hour}:{datetime.datetime.now().hour}"
        extraction_time_start = time_start
        extraction_time_stop = time_stop
        elapsed_time = time_start - time_stop

        metadata = [property_type, ads_quantity, ]
